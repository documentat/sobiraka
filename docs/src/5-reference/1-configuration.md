...
_Конфиг проекта_ — файл в формате YAML, который определяет все настройки [проекта](../1-concepts/1-volumes.md).
...
Директория, файлы из которой можно будет включать в другие файлы, см. [инклюды](../2-syntax/4-jinja.md#includes).
...
Если `true`, то в начало каждого заголовка в документации будет добавлен его порядковый номер. См. [](../1-concepts/3-structure.md#autonumeration).

### `html.toc_depth` {#volume.html.toc_depth}

Глубина, до которой должны быть раскрыты неактивные пункты оглавления. Не применяется к активному пункту и всем его родительским пунктам. См. [](../1-concepts/3-structure.md#toc).
...
Эта опция позволяет встраивать локальное оглавление страницы внутрь глобального оглавления. См. [](../1-concepts/3-structure.md#toc).
...
- внутри страниц с помощью [синтаксиса Jinja](../2-syntax/4-jinja.md), например: `{{ '{{ MYVAR }}' }}`,
