...
### `pdf.headers` {#volume.pdf.headers}

Правила выбора команд LaTeX для заголовков в документе. Правила могут быть заданы в четырёх словарях. Собирака ищет правила в словарях в следующем порядке и по следующим ключам.

1. `by_class`: ключом является имя класса, добавленное к заголовку (например, `{.part}` в синтаксисе Markdown);
1. `by_global_level`: ключом является уровень заголовка внутри глобальной иерархии всего тома (например, на странице 3-го уровня заголовок H2 будет иметь уровень 4);
1. `by_page_level`: ключом является уровень страницы (это правило срабатывает только для заголовков первого уровня);
1. `by_element`: ключом является уровень заголовка внутри страницы в формате `h1`, `h2`, и так далее;

Из-за особенностей синтаксиса YAML числовые ключи необходимо заключать в кавычки — например, писать `'1'` вместо `1`.

Для каждого заголовка срабатывает первое найденное правило.

По умолчанию Собирака использует конфигурацию, показанную ниже. Возможно, вам понадобится переопределить `by_global_level` пустым словарём, чтобы срабатывали другие необходимые вам правила.
