...
_Конфиг проекта_ — файл в формате YAML, который определяет все настройки [проекта](../1-overview/03-volumes.md).
...
Язык, который используется в качестве первоисточника при переводе томов, см. [Мультиязычность](../1-overview/04-multilang.md).
...
Если `true`, то в начало каждого заголовка в документации будет добавлен его порядковый номер. См. [](../1-overview/02-structure.md#autonumeration).

### `html.toc_depth` {#volume.html.toc_depth}

Глубина, до которой должны быть раскрыты неактивные пункты оглавления. Не применяется к активному пункту и всем его родительским пунктам. См. [](../1-overview/02-structure.md#toc).
...
Эта опция позволяет встраивать локальное оглавление страницы внутрь глобального оглавления. См. [](../1-overview/02-structure.md#toc).
...
- `$AUTOPREFIX` — [идентификатор тома](../1-overview/03-volumes.md#volume-id) (в общем случае то же, что `$LANG/$VOLUME`).
