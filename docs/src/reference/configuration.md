...
### Два языка, два документа {#example-2l-2d}

Данный пример описывает проект, содержащий два языка (`en` и `ru`) и два документа (`admin-manual` и `user-manual`). Каждая языковая версия каждого документа имеет собственные настройки [`title`](#title) и [`paths.root`](#paths.root).
...
### Два языка, один документ {#example-2l-1d}

Данный пример описывает проект из одного документа, представленного на двух языках (`en` и `ru`). В отличие от примера [«Два языка, два документа»](#example-2l-2d), здесь не используется конструкция `documents`: вместо этого ключам `en` и `ru` соответствуют непосредственно настройки документа.
...
### Один язык, два документа {#example-1l-2d}

Данный пример описывает проект, содержащий два документа (`admin-manual` и `user-manual`) на одном языке. В отличие от примера [«Два языка, два документа»](#example-2l-2d), здесь опускается конструкция `languages`, а конструкция `documents` вынесена на верхний уровень.
...
### Один язык, один документ {#example-1l-1d}
...
- `$AUTOPREFIX` — [идентификатор документа](../overview/terms.md#document) (в общем случае то же, что `$LANG/$DOCUMENT`).

Например, конфигурация ниже полностью идентична конфигурации из раздела [«Два языка, один документ»](#example-2l-1d), но использует переменную `$DOCUMENT`, чтобы сформировать нужный путь `root` для каждого документа.
...
Язык, который используется в качестве первоисточника при переводе документов, см. [Мультиязычность](../overview/multilang.md).

Если настройка не задана, то используется первый язык в порядке объявления в конфиге.

## Основные настройки документа {#document-configuration}
...
Для поиска файлов по шаблонам используются функции из библиотеки [`wcmatch`](https://facelessuser.github.io/wcmatch/glob/) с включёнными опциями `GLOBSTAR` и `NODIR`. Если указано несколько шаблонов, то в документ попадут файлы, соответствующие хотя бы одному из них (и не соответствующие ни одному из шаблонов [`paths.exclude`](#paths.exclude)). Если настройка не задана, то используется один шаблон `**/*`, которому соответствуют все файлы на любой глубине.
...
Для поиска файлов по шаблонам используются функции из библиотеки [`wcmatch`](https://facelessuser.github.io/wcmatch/glob/) с включёнными опциями `GLOBSTAR` и `NODIR`. Если указано несколько шаблонов, то из документа будут исключены файлы, соответствующие хотя бы одному из них. Если настройка не задана, то никакие файлы не будут исключены.
...
- Если группа `is_main` не пуста, то файл считается [индексным](../organizing/files.md#directories-and-index-files) для своей директории и всегда отображается в ней первым.
...
Если настройка не задана, она считается равной `$AUTOPREFIX`, что означает автоматическое создание поддиректорий для каждого языка, а внутри — для каждого документа. См. [](#default).
...
Путь к файлу Python, в котором определён [обработчик страниц](processor-api.md) для документа.
