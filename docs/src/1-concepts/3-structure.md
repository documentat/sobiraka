...
По умолчанию главная страница раздела содержит автоматически сгенерированное [оглавление раздела](#toc). Его можно заменить на любой другой контент, создав в папке файл с именем `index.md` (это имя можно изменить через настройку [`paths.naming_scheme`](../5-data-types/1-manifest.md#volume.paths.naming_scheme)).
