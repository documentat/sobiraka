...
from sobiraka.models import DirPage, Page, Project, Volume


class TestInitProject(TestCase):

    def test_with_base_tuple(self):
        base = Path('/project')
        ...
        with self.subTest('base'):
            self.assertEqual(base, project.base)
    ...
    def test_with_base_dict(self):
        ...
        with self.subTest('roots'):
            expected = Path('/project/src1'), Path('/project/src2'), Path('/project/src3')
            actual = tuple(v.root for v in project.volumes)
            self.assertSequenceEqual(expected, actual)
